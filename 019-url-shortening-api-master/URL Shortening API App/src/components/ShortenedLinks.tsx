  ...
  return (
    <section className="flex flex-col gap-4 mt-28 z-10 w-full max-w-[1120px]">
      {shortenedLinks.map((link, i) => {
        return (
          <div key={i+1001} id={`link-${i}`} className="flex flex-col md:flex-row md:items-center bg-white py-4 rounded -translate-x-3 ">
            ...
            <button id={"copyB"} className="bg-cyan text-white mx-4 rounded-md py-2 mt-4 flex-grow font-bold md:mt-0 md:self-start md:w-[112px] transition" onClick={() => copyLink(i)}>Copy</button>
          </div>
        );
      })}
    </section>
